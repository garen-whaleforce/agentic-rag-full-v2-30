#!/usr/bin/env python3
"""
Run batch test for 1000 additional samples for garen1212v4.
Will merge results with previous 2013 successful samples.
"""
...
INPUT_FILE = Path(__file__).parent / "batch_1000_additional_garen1212v4_samples.json"
PREVIOUS_MERGED_FILE = Path(__file__).parent / "batch_2500_garen1212v4_final_20251214_095748.json"
...
async def run_batch():
    ...
    output_json = OUTPUT_DIR / f"batch_1000_additional_{timestamp}.json"
    final_merged_json = OUTPUT_DIR / f"batch_3500_garen1212v4_final_{timestamp}.json"
    final_merged_csv = OUTPUT_DIR / f"batch_3500_garen1212v4_final_{timestamp}.csv"
    ...
    total = len(samples)
    print(f"=" * 70)
    print(f"GAREN1212V4 - 1000 Additional Samples")
    print(f"=" * 70)
    ...
    print(f"\n{'='*70}")
    print(f"1000 ADDITIONAL SAMPLES RESULTS")
    print(f"{'='*70}")
    ...
    with open(output_json, 'w') as f:
        json.dump(sorted_results, f, indent=2, ensure_ascii=False)
    print(f"\nSaved 1000 results: {output_json}")
    ...
    print(f"\n{'='*70}")
    print(f"FINAL MERGED RESULTS (2500 + 1000)")
    print(f"{'='*70}")
