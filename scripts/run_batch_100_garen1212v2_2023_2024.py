...
ONLY 2023 and 2024 data - excludes COVID period for cleaner comparison.
...
YEAR_FILTER_PRIMARY = [2023, 2024]  # Primary years
YEAR_FILTER_BACKUP = [2025]  # Backup year if not enough data in primary years
...
def load_test_cases() -> List[Dict]:
    """Load test cases from both CSV files - only top N from each, filtered by year"""
    test_cases = []

    print(f"\nFiltering for years: {YEAR_FILTER_PRIMARY} (backup: {YEAR_FILTER_BACKUP})", flush=True)

    # Load top N gainers (filtered by year)
    with open(GAINERS_CSV, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # Filter by primary years
            if fiscal_year not in YEAR_FILTER_PRIMARY:
                continue
        ...
        print(f"  Gainers: Scanned {scanned} rows, found {count} matching {YEAR_FILTER_PRIMARY}", flush=True)

    # Load top N losers (filtered by year) - with backup from 2022 if needed
    loser_cases = []
    with open(LOSERS_CSV, 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        count_primary = 0
        scanned = 0
        all_rows = list(reader)  # Read all rows first

        # First pass: get primary years (2023, 2024)
        for row in all_rows:
            scanned += 1
            if count_primary >= TOP_N:
                break
        ...
        print(f"  Losers (primary): Scanned {scanned} rows, found {count_primary} matching {YEAR_FILTER_PRIMARY}", flush=True)

        # Second pass: fill remaining with backup year (2022) if needed
        remaining = TOP_N - count_primary
        if remaining > 0:
            count_backup = 0
            for row in all_rows:
                if count_backup >= remaining:
                    break

                earnings_date = row['earnings_date']
                fiscal_year, fiscal_quarter = determine_fiscal_quarter(earnings_date)

                # Filter by backup years
                if fiscal_year not in YEAR_FILTER_BACKUP:
                    continue
            ...
            print(f"  Losers (backup): Added {count_backup} from {YEAR_FILTER_BACKUP}", flush=True)

    test_cases.extend(loser_cases)
    print(f"  Losers total: {len(loser_cases)}", flush=True)
...
def print_summary_report(results: List[Dict], elapsed_seconds: float, total: int, is_final: bool = False):
    ...
    print(f"Year Filter: {YEAR_FILTER} | Elapsed: {elapsed_seconds/60:.1f} minutes | Processed: {len(results)}/{total}", flush=True)
...
def save_results(results: List[Dict], timestamp: str):
    ...
    years_str = "_".join(map(str, YEAR_FILTER))
    csv_path = Path(__file__).resolve().parent / f"batch_100_garen1212v2_{years_str}_{timestamp}.csv"
    ...
    # JSON with full data
    json_path = Path(__file__).resolve().parent / f"batch_100_garen1212v2_{years_str}_{timestamp}.json"
...
async def main():
    ...
    print(f"Year Filter: Primary {YEAR_FILTER_PRIMARY}, Backup {YEAR_FILTER_BACKUP} (excluding COVID period)", flush=True)
    print(f"Test Size: {TOP_N} gainers + {TOP_N} losers = {TOP_N*2} total (if available)", flush=True)
