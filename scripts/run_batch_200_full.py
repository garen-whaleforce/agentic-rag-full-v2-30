#!/usr/bin/env python3
"""
Batch test for 200 tickers (100 gainers + 100 losers) with full output fields.
...
def get_test_list() -> List[Dict]:
    """Combine gainers and losers into test list - all 200 entries."""
    test_list = []
    # Add all 100 gainers
...
def determine_result(predicted: str, actual: Optional[float]) -> str:
    ...
    if pred_upper == "UP":
        ...
        return "SKIP"  # NEUTRAL = don't trade, skip
    return "N/A"
...
def print_summary_report(results: List[Dict], elapsed_seconds: float, is_final: bool = False):
    ...
    if success:
        ...
        print(f"By Category: GAINER={len(gainers)}, LOSER={len(losers)}", flush=True)

        # Accuracy calculation (NEUTRAL = SKIP, not counted)
        ...
        print(f"\nResults: CORRECT={len(correct)}, WRONG={len(wrong)}, SKIP(NEUTRAL)={len(skip)}", flush=True)
        ...
        # Print table header
        print(f"\n{'#':<4} {'ticker':<6} {'year':<5} {'Q':<2} {'date':<12} {'cat':<7} {'exp':<5} {'pred':<8} {'conf':<5} {'actual':<8} {'result':<8} {'sec':<6}", flush=True)
        ...
        for r in results[-20:]:
            if r.get("status") == "success":
                ...
                print(f"{r.get('rank', '-'):<4} {r.get('ticker', '-'):<6} {r.get('year', '-'):<5} Q{r.get('quarter', '-'):<1} {r.get('date', '-'):<12} {r.get('category', '-'):<7} {r.get('expected_pred', '-'):<5} {r.get('predicted', '-'):<8} {conf_str:<5} {actual_str:<8} {r.get('result', '-'):<8} {r.get('elapsed', 0):.0f}", flush=True)
    ...
    async with semaphore:
        ...
        try:
            ...
            if analysis_result and "error" not in analysis_result:
                ...
                print(f"  {ticker}: {result_row['predicted']} (conf={result_row['confidence']}) actual={actual_from_pdf}% -> {result_row['result']} [{elapsed:.0f}s]", flush=True)
            else:
                error_msg = analysis_result.get("error", "Unknown") if analysis_result else "No result"
                result_row["status"] = "ERROR"
                result_row["error"] = error_msg[:100]
                print(f"  {ticker}: ERROR - {error_msg[:50]} [{elapsed:.0f}s]", flush=True)
            ...
            result_row["elapsed"] = elapsed
            print(f"  {ticker}: EXCEPTION - {str(e)[:50]} [{elapsed:.0f}s]", flush=True)
...
async def main():
    ...
    csv_file = Path(__file__).parent / f"batch_200_results_{timestamp}.csv"
    save_results_csv(results_list, csv_file)

    json_file = Path(__file__).parent / f"batch_200_results_{timestamp}.json"
