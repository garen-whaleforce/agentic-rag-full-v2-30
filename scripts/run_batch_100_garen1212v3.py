#!/usr/bin/env python3
"""
Batch test for top 100 earnings moves (50 gainers + 50 losers) using garen1212v3 profile.
Smaller test set for faster validation.

garen1212v3 improvements over v2:
...
# Configuration
PROFILE_NAME = "garen1212v3"  # Evidence-gated T+30 profile
CONCURRENCY = 15
...
def save_results(results: List[Dict], timestamp: str):
    ...
    csv_path = Path(__file__).resolve().parent / f"batch_100_garen1212v3_{timestamp}.csv"
    ...
    # JSON with full data
    json_path = Path(__file__).resolve().parent / f"batch_100_garen1212v3_{timestamp}.json"
...
async def main():
    ...
    print(f"BATCH TEST - Top {TOP_N*2} Earnings Moves with garen1212v3 Profile (Balanced T+30)", flush=True)
