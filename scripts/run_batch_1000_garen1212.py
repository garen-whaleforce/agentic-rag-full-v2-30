#!/usr/bin/env python3
"""
Batch test for top 1000 earnings moves (500 gainers + 500 losers) using garen1212 profile.
Uses the provided CSVs with pre-calculated T+30 data.

Features:
- Applies garen1212 profile (T+30 optimized prompts)
...
# Configuration
PROFILE_NAME = "garen1212"  # T+30 optimized profile
CONCURRENCY = 15
...
def print_summary_report(results: List[Dict], elapsed_seconds: float, total: int, is_final: bool = False):
    ...
    if success:
        ...
        if scores:
            ...
            print(f"\nDirection Scores: Avg={avg_score:.2f} | 0-3={low_scores} | 4-6={mid_scores} | 7-10={high_scores}", flush=True)
...
def save_results(results: List[Dict], timestamp: str):
    ...
    csv_path = Path(__file__).resolve().parent / f"batch_1000_garen1212_{timestamp}.csv"
    ...
    # JSON with full data
    json_path = Path(__file__).resolve().parent / f"batch_1000_garen1212_{timestamp}.json"
...
async def main():
    ...
    print("BATCH TEST - Top 1000 Earnings Moves with garen1212 Profile (T+30)", flush=True)
